...
class 场景_结算(场景基类):
    ...
    def _取流程阶段(self, 经过秒: float) -> Tuple[int, float]:
        ...
        流程2终点 = 流程1终点 + float(self._取流程2有效时长秒())
        if 经过秒 < 流程1终点:
    ...
    def _取流程2动画时长秒(self) -> float:
        if not self._是否显示奖励小窗():
            return 0.0
        ...
        for 模式名 in ("花式", "竞速"):
            ...
            for 片段 in 进度片段:
                try:
                    if float(片段.get("增长", 0.0) or 0.0) > 1e-6:
                        有效片段数 += 1
                except Exception:
                    continue
            最大片段数 = max(最大片段数, int(有效片段数))
            最大升级数 = max(最大升级数, int(数据.get("升级次数", 0) or 0))

        动画开始秒 = float(self._经验窗入场秒) + 0.20
        if 最大片段数 > 0:
            进度动画秒 = float(self._流程2单段动画秒) * float(最大片段数)
        elif int(奖励.get("经验增加值", 0) or 0) > 0:
            进度动画秒 = float(self._流程2单段动画秒)
        else:
            进度动画秒 = 0.55

        if bool(奖励.get("显示升级动画", False)) and 最大升级数 > 0:
            进度动画秒 += float(self._流程2升级附加秒) * float(最大升级数)

        return float(
            min(
                float(self._流程2动画最大时长秒),
                max(
                    float(self._流程2最短时长秒),
                    float(动画开始秒) + float(进度动画秒) + 0.35,
                ),
            )
        )

    def _取流程2有效时长秒(self) -> float:
        动画时长秒 = float(self._取流程2动画时长秒())
        if 动画时长秒 <= 0.0:
            return 0.0
        return float(动画时长秒 + max(0.0, float(self._流程2收尾停留秒)))
    ...
    def _计算经验动画状态(self, 模式名: str, 经过秒: float) -> Dict[str, object]:
        ...
        流程2动画时长秒 = max(
            float(动画开始秒) + 0.35,
            float(self._取流程2动画时长秒()),
        )
        动画时长 = max(0.30, float(流程2动画时长秒) - 动画开始秒 - 0.35)
        ...
        for 片段 in 进度片段:
            try:
                总增长 += max(0.0, float(片段.get("增长", 0.0) or 0.0))
            except Exception:
                continue

        if 总增长 > 1e-6:
            目标消费 = float(总增长) * float(进度)
            已消费 = 0.0
            for 片段 in 进度片段:
                try:
                    片段增长 = max(0.0, float(片段.get("增长", 0.0) or 0.0))
                except Exception:
                    片段增长 = 0.0
                if 片段增长 <= 1e-6:
                    continue
                ...
                片段起点 = float(已消费)
                片段终点 = float(已消费 + 片段增长)
                是否升级 = bool(片段.get("是否升级", False))
                if 是否升级:
                    升级节点.append(float(片段终点 / max(1e-6, 总增长)))
                ...
                if 目标消费 >= 片段终点 - 1e-6:
                    ...
                    当前经验 = min(float(当前经验), float(当前级所需经验))
                    break
                else:
                    break
        ...
        升级动画t = None
        if bool(奖励.get("显示升级动画", False)) and 升级节点:
            升级动画时长秒 = min(
                0.90,
                max(0.55, float(动画时长) / max(1.0, float(len(升级节点)) * 1.15)),
            )
            for 升级节点进度 in reversed(升级节点):
                升级触发秒 = float(动画开始秒) + float(动画时长) * float(升级节点进度)
                升级动画开始秒 = min(
                    升级触发秒,
                    float(流程2动画时长秒) - float(升级动画时长秒),
                )
        ...
        return {
            "等级": int(max(1, 当前等级)),
            "经验": float(max(0.0, 当前经验)),
            "经验比例": float(计算经验显示比例(当前等级, 当前经验)),
            "进度": float(进度),
            "升级动画t": 升级动画t,
            "升级次数": int(max(0, len(升级节点))),
        }
    ...
    def _规范化结算进度(self, 进度: dict) -> tuple[dict, dict, dict]:
        进度 = dict(进度 or {})
        最大等级 = max(1, min(70, int(进度.get("最大等级", 70) or 70)))
    ...
    def _构建模式奖励数据(
        ...
        原模式进度: dict,
        新模式进度: dict,
        经验结算: Optional[dict] = None,
    ) -> dict:
        原模式进度 = 规范化模式进度(原模式进度, 经验版本=经验数据版本)
        新模式进度 = 规范化模式进度(新模式进度, 经验版本=经验数据版本)
        经验结算 = dict(经验结算 or {})
        return {
            "原等级": int(原模式进度.get("等级", 1) or 1),
            "原经验": float(原模式进度.get("经验", 0.0) or 0.0),
            "原等级所需经验": int(取升下一级所需经验(原模式进度.get("等级", 1))),
            "等级": int(新模式进度.get("等级", 1) or 1),
            "经验": float(新模式进度.get("经验", 0.0) or 0.0),
            "等级所需经验": int(取升下一级所需经验(新模式进度.get("等级", 1))),
            "进度片段": list(经验结算.get("进度片段", []) or []),
            "升级次数": int(经验结算.get("升级次数", 0) or 0),
            "是否满级": bool(经验结算.get("是否满级", False))
            or int(新模式进度.get("等级", 1) or 1) >= 70,
        }
    ...
    def _更新个人资料(self):
        ...
        cool数 = int(self._载荷.get("cool数", 0) or 0)
        good数 = int(self._载荷.get("good数", 0) or 0)
        miss数 = int(self._载荷.get("miss数", 0) or 0)
        ...
        进度["最大等级"] = int(max(int(进度.get("最大等级", 70) or 70), 最高等级))
        ...
        当前模式经验结算 = (
            dict(经验结算结果.get("经验结算", {}) or {})
            if isinstance(经验结算结果.get("经验结算", {}), dict)
            else {}
        )
        奖励窗可见 = bool(经验结算结果.get("显示奖励窗", False))
        升级模式 = ""
        if 奖励窗可见 and int(当前模式经验结算.get("升级次数", 0) or 0) > 0:
            升级模式 = 模式键

        self._奖励数据 = {
            "经验增加值": int(经验结算结果.get("经验增加值", 0) or 0),
            "是否升级": bool(
                奖励窗可见 and int(当前模式经验结算.get("升级次数", 0) or 0) > 0
            ),
            "升级模式": 升级模式,
            "显示奖励窗": bool(奖励窗可见),
            "显示升级动画": bool(
                奖励窗可见
                and bool(经验结算结果.get("显示升级动画", False))
                and int(当前模式经验结算.get("升级次数", 0) or 0) > 0
            ),
            "结算类型": str(经验结算结果.get("结算类型", "") or ""),
            "正式局歌曲": list(经验结算结果.get("正式局歌曲", []) or []),
            "单首歌结果": dict(经验结算结果.get("单首歌结果", {}) or {}),
            "花式": self._构建模式奖励数据(
                原花式进度,
                花式进度,
                当前模式经验结算 if 模式键 == "花式" else None,
            ),
            "竞速": self._构建模式奖励数据(
                原竞速进度,
                竞速进度,
                当前模式经验结算 if 模式键 == "竞速" else None,
            ),
            "段位路径": 段位相对路径,
        }
    ...
    def _是否显示奖励小窗(self) -> bool:
        return bool((self._奖励数据 or {}).get("显示奖励窗", False))
    ...
    def _快进到流程3可见(self, 当前系统秒: Optional[float] = None):
        if 当前系统秒 is None:
            当前系统秒 = time.perf_counter()
        流程2终点 = float(self._流程1时长秒) + float(self._取流程2有效时长秒())
    ...
    def _绘制奖励小窗(
        ...
        self._绘制经验进度区(
            画布,
            小窗区域,
            经过秒 if not 固定最终 else float(self._取流程2动画时长秒()),
            实际屏幕尺寸,
        )
