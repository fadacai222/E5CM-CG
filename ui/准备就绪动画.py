...
def 绘制准备就绪动画(
    ...
    if 基础场景图 is not None:
        ...
        if 血条组t > 0.0 and 顶部HUD矩形.w > 0 and 顶部HUD矩形.h > 0:
            try:
                ...
                偏移y = int((1.0 - _ease_out_cubic(血条组t)) * -float(顶部HUD矩形.h + 36))
    ...
    引导出场t = _clamp01((经过秒 - 引导出场开始) / max(0.001, 引导出场结束 - 引导出场开始))
    ...
    if 背板1 is not None:
        ...
        板淡出t = _clamp01((经过秒 - 引导出场开始) / max(0.001, 引导出场结束 - 引导出场开始))
        ...
        if 板alpha > 0 and 板t > 0.0:
            ...
            if 板rect is not None and 背板2 is not None:
                try:
                    ...
                    叠图宽 = int(max(1, 背板2.get_width() * (叠图高 / max(1, 背板2.get_height()))))
                    单张 = _取缩放缓存图(
                        缓存,
