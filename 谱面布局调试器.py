...
def 主函数():
    ...
    def _配置调试渲染器():
        ...
        try:
            ...
            pass
        try:
            绑定布局管理器 = getattr(模拟hold渲染器, "绑定布局管理器", None)
            if callable(绑定布局管理器):
                绑定布局管理器(布局管理器)
            else:
                setattr(模拟hold渲染器, "_布局管理器_谱面渲染器", 布局管理器)
        except Exception:
            pass
    ...
    def _计算文本字间距面板布局():
        if (not bool(文本字间距面板开启)) or (
            not _是否文本控件(文本字间距目标控件id)
        ):
            return None, None, None, None
        面板宽 = 360
        面板高 = 96
        屏宽, 屏高 = 屏幕.get_size()
        锚点x = int(文本字间距面板锚点[0]) if 文本字间距面板锚点 else 屏宽 // 2
        锚点y = int(文本字间距面板锚点[1]) if 文本字间距面板锚点 else 屏高 // 2
        x = int(max(12, min(锚点x - 面板宽 // 2, 屏宽 - 面板宽 - 12)))
        候选y = int(锚点y + 18)
        if 候选y + 面板高 > 屏高 - 12:
            候选y = int(锚点y - 面板高 - 18)
        y = int(max(12, min(候选y, 屏高 - 面板高 - 12)))
    ...
    def _取开关项():
        return [
            ...
            ("loop_hit_fx", "持续播放击中动画", bool(持续播放击中动画)),
        ]
    ...
    def _构建调试渲染输入(
        基础上下文: Dict[str, Any], 玩家序号: int = 1
    ) -> Optional[Any]:
        if 模拟hold渲染器 is None or 渲染输入类 is None:
            return None
        if not isinstance(基础上下文, dict):
            return None
        ...
        try:
            游戏缩放 = float((参数 or {}).get("缩放", 1.0) or 1.0)
        except Exception:
            游戏缩放 = 1.0
        try:
        ...
        try:
            轨道中心列表 = [
                int(float(v))
                for v in list(基础上下文.get("调试_hold_轨道中心列表", []) or [])[:5]
            ]
        except Exception:
            轨道中心列表 = []
        if len(轨道中心列表) < 5:
        ...
        try:
            判定线y = int(float(基础上下文.get("调试_hold_判定线y", 0) or 0))
        except Exception:
            判定线y = 0
        try:
            底部y = int(float(基础上下文.get("调试_hold_底部y", 0) or 0))
        except Exception:
            底部y = 0
        try:
            hold箭头宽 = float(基础上下文.get("调试_hold_箭头宽", 80) or 80)
        except Exception:
            hold箭头宽 = 80.0

        箭头目标宽 = int(
            max(28, round(float(hold箭头宽) / max(0.01, 游戏缩放 * hold宽度系数)))
        )
        血条区域 = pygame.Rect(0, 0, int(max(96, 屏幕.get_width() * 0.28)), 40)
        当前谱面秒 = float(基础上下文.get("当前谱面秒", 0.0) or 0.0)

        输入 = 渲染输入类(
            当前谱面秒=float(当前谱面秒),
            总时长秒=180.0,
            轨道中心列表=list(轨道中心列表[:5]),
            判定线y=int(判定线y),
            底部y=int(底部y),
            滚动速度px每秒=float(max(60.0, 420.0 * float(模拟谱面调速倍率))),
            箭头目标宽=int(箭头目标宽),
            事件列表=[],
            显示_判定="perfect",
            显示_连击=int(max(0, int(基础上下文.get("计数_combo", 23) or 23))),
            显示_分数=int(max(0, int(基础上下文.get("显示_分数", 123456) or 123456))),
            显示_百分比="100.00%",
            血条区域=血条区域,
            血量显示=float(基础上下文.get("血量最终显示", 调试血量显示) or 调试血量显示),
            头像图=头像图,
            总血量HP=int(max(0, int(基础上下文.get("总血量HP", 0) or 0))),
            可见血量HP=int(max(0, int(基础上下文.get("可见血量HP", 0) or 0))),
            血条暴走=bool(基础上下文.get("血条暴走", False)),
            玩家序号=int(玩家序号),
            玩家昵称=str(基础上下文.get("玩家昵称", 玩家昵称) or 玩家昵称),
            段位图=段位图,
            当前关卡=int(max(1, int(基础上下文.get("当前关卡", 1) or 1))),
            歌曲名=str(基础上下文.get("歌曲名", 调试歌曲名) or 调试歌曲名),
            星级=7,
            轨迹模式=str(模拟谱面轨迹模式 or "正常"),
            隐藏模式=str(模拟谱面隐藏模式 or "关闭"),
            大小倍率=1.0,
            圆环频谱对象=调试圆环频谱对象,
        )
        setattr(输入, "调试_计数常显", True)
        setattr(输入, "调试_计数判定", "perfect")
        setattr(输入, "调试_计数combo", int(max(0, int(基础上下文.get("计数_combo", 23) or 23))))

        try:
            取锚点 = getattr(模拟hold渲染器, "_取判定区实际锚点", None)
            实际锚点 = 取锚点(屏幕, 输入) if callable(取锚点) else None
        except Exception:
            实际锚点 = None

        if isinstance(实际锚点, dict):
            try:
                实际轨道中心列表 = [
                    int(v)
                    for v in list(实际锚点.get("轨道中心列表", []) or [])[:5]
                ]
                if len(实际轨道中心列表) >= 5:
                    输入.轨道中心列表 = list(实际轨道中心列表[:5])
            except Exception:
                pass
            try:
                输入.判定线y = int(实际锚点.get("判定线y", 输入.判定线y) or 输入.判定线y)
            except Exception:
                pass
            try:
                判定区宽度列表 = [
                    float(v)
                    for v in list(实际锚点.get("判定区宽度列表", []) or [])[:5]
                ]
                if 判定区宽度列表:
                    目标判定区宽 = float(
                        判定区宽度列表[2]
                        if len(判定区宽度列表) >= 3
                        else sum(判定区宽度列表) / float(len(判定区宽度列表))
                    )
                    输入.箭头目标宽 = int(
                        max(
                            28,
    ...
    def _同步判定区锚点(上下文: Dict[str, Any]):
        输入 = _构建调试渲染输入(
            上下文, 玩家序号=int(上下文.get("玩家序号", 1) or 1)
        )
        if 输入 is None or 模拟hold渲染器 is None:
            return

        try:
            取锚点 = getattr(模拟hold渲染器, "_取判定区实际锚点", None)
            锚点 = 取锚点(屏幕, 输入) if callable(取锚点) else None
        except Exception:
            锚点 = None
        if not isinstance(锚点, dict):
            return
        ...
        try:
            轨道中心列表 = [
                int(v) for v in list(锚点.get("轨道中心列表", []) or [])[:5]
            ]
        except Exception:
            轨道中心列表 = []
        try:
            判定线y列表 = [
                int(v) for v in list(锚点.get("判定线y列表", []) or [])[:5]
            ]
        except Exception:
            判定线y列表 = []
        try:
            判定区宽列表 = [
                int(max(2, float(v)))
                for v in list(锚点.get("判定区宽度列表", []) or [])[:5]
            ]
        except Exception:
            判定区宽列表 = []

        if len(轨道中心列表) >= 5:
            上下文["调试_hold_轨道中心列表"] = list(轨道中心列表[:5])
        if 判定线y列表:
            上下文["调试_hold_判定线y列表"] = list(判定线y列表[:5])
        ...
        if 判定区宽列表:
            箭头宽列表 = [
                int(max(16, round(float(v) * 宽度比))) for v in 判定区宽列表[:5]
            ]
            上下文["调试_hold_箭头宽列表"] = 箭头宽列表
            上下文["调试_hold_箭头宽"] = int(
                箭头宽列表[2] if len(箭头宽列表) >= 3 else 箭头宽列表[0]
            )

    def _更新调试击中特效预览(当前秒: float):
        if 模拟hold渲染器 is None:
            return
        try:
            设置预览 = getattr(模拟hold渲染器, "设置调试击中特效预览", None)
            if callable(设置预览):
                设置预览(bool(持续播放击中动画), float(当前秒), 轨道序号=2)
        except Exception:
            pass
    ...
    def _复制上下文并隐藏控件(
        上下文: Optional[Dict[str, Any]], 控件id列表: List[str]
        ...
        try:
            隐藏集合 = {
                str(v)
                for v in list(结果.get("_调试隐藏控件ids", []) or [])
                if str(v or "").strip()
            }
        except Exception:
            隐藏集合 = set()
        for 控件id in 控件id列表:
            if str(控件id or "").strip():
                隐藏集合.add(str(控件id))
        结果["_调试隐藏控件ids"] = sorted(list(隐藏集合))
        return 结果
    ...
    def _取调试渲染输入列表(
        ...
        批次: List[Tuple[str, Dict[str, Any], Any]] = []
        if not bool(模拟双踏板模式):
            输入 = _构建调试渲染输入(主上下文, 玩家序号=1)
            if 输入 is not None:
                批次.append(("主", 主上下文, 输入))
            return 批次
        ...
        for 标签, 子上下文, 玩家序号 in (
            ...
            if not isinstance(子上下文, dict):
                continue
            输入 = _构建调试渲染输入(子上下文, 玩家序号=玩家序号)
            if 输入 is not None:
                批次.append((标签, 子上下文, 输入))
        return 批次
    ...
    def _取调试渲染矩形项(
        ...
        if 模拟hold渲染器 is None:
            return []
        结果: List[Tuple[str, pygame.Rect]] = []
        for _标签, _上下文, 输入 in _取调试渲染输入列表(基础上下文):
            try:
                取矩形表 = getattr(模拟hold渲染器, "取调试控件组矩形表", None)
                矩形表 = 取矩形表(屏幕, 输入) if callable(取矩形表) else {}
            except Exception:
                矩形表 = {}
            if not isinstance(矩形表, dict):
                continue
            for 控件id in ("计数动画组", "特效层组"):
                矩形 = 矩形表.get(控件id)
                if isinstance(矩形, pygame.Rect):
                    结果.append((str(控件id), 矩形.copy()))
        return 结果

    def _命中调试渲染控件(鼠标点: Tuple[int, int], 基础上下文: Dict[str, Any]) -> str:
        点x, 点y = int(鼠标点[0]), int(鼠标点[1])
        for 控件id, 矩形 in _取调试渲染矩形项(基础上下文):
            try:
    ...
    def _绘制调试渲染组边框(基础上下文: Dict[str, Any]):
        if (not bool(显示全部边框)) and str(选中控件id or "") not in {
            "特效层组",
            "计数动画组",
        }:
            return
        ...
        for 控件id, 矩形 in _取调试渲染矩形项(基础上下文):
            try:
                ...
                if 控件id == "计数动画组":
                    颜色 = (245, 208, 118)
                if str(选中控件id or "") == str(控件id):
                    高亮底 = pygame.Surface((矩形.w, 矩形.h), pygame.SRCALPHA)
                    高亮底.fill((90, 110, 160, 78))
                    屏幕.blit(高亮底, 矩形.topleft)
                    pygame.draw.rect(屏幕, (255, 240, 160), 矩形, width=2, border_radius=8)
                elif bool(显示全部边框):
                    pygame.draw.rect(屏幕, 颜色, 矩形, width=1, border_radius=8)
            except Exception:
                continue

    def _绘制调试渲染特效组(基础上下文: Dict[str, Any]):
        if 模拟hold渲染器 is None:
            return
        for _标签, _上下文, 输入 in _取调试渲染输入列表(基础上下文):
            try:
                绘制函数 = getattr(模拟hold渲染器, "绘制调试特效与计数组", None)
                if callable(绘制函数):
                    绘制函数(屏幕, 输入)
            except Exception:
                continue
    ...
    def _取图层项():
        try:
            return [
                项
                for 项 in list(布局管理器.取调试图层列表())
                if not str((项 or {}).get("id") or "").strip().startswith("击中特效_")
            ]
        except Exception:
            return []

    def _规范调试控件id(控件id: str) -> str:
        控件id = str(控件id or "").strip()
        if 控件id.startswith("击中特效_"):
            return "特效层组"
        return 控件id
    ...
    while True:
        时间差秒 = float(时钟.tick(60)) / 1000.0
        当前秒 = float(pygame.time.get_ticks()) / 1000.0
        _更新调试击中特效预览(当前秒)
        if 模拟hold渲染器 is not None:
            try:
                getattr(模拟hold渲染器, "更新")(时间差秒)
            except Exception:
                pass
                ...
                if 事件.button == 1:
                    ...
                    if (not bool(左侧面板折叠)) and 面板rect.collidepoint(事件.pos):
                        ...
                        if 行rect表.get("show_all_borders") and 行rect表[
                            ...
                            _保存调试设置()
                        elif 行rect表.get("loop_hit_fx") and 行rect表[
                            "loop_hit_fx"
                        ].collidepoint(事件.pos):
                            选中调试项id = ""
                            持续播放击中动画 = not bool(持续播放击中动画)
                            _保存调试设置()
                        拖拽中 = False
                        拖拽已记录撤销 = False
                        continue
